...
这是进程内按顺序 await 的分发器，不是外部消息中间件。慢任务应先进入业务自己的持久队列并迅速返回；看板发布已有这种实现。监听清单通过代码注册，当前没有在线编辑、动态导入插件或浏览器开关。

## 处理顺序

1. HTTP 层接收原始字节，Pipeline 验签并解析 JSON／表单。
2. 适配器生成统一 `BotEvent`；Pipeline 检查全局仓库范围及投递去重。
...
存档发生在处理之后；总线不提供事务或全局“恰好一次”。进程在业务执行后、存档前退出时可能再次执行业务。单进程 Pipeline 将检查、分发和归档串行化，避免并发投递重复通过去重检查；总线向每个回调提供独立事件副本。处理器仍应在外部副作用处实现幂等，不要仅依赖进程内状态防止重复写入。

## 注册一个业务

在业务模块实现回调，在 `src/core/bus.ts` 的 `registerBuiltin()` 组合入口注册；也可在构建 Router 后、服务启动前使用 `router.bus.subscribe()`：

```typescript
import type { BotEvent } from '../../src/core/types.js';

async function enqueueNotification(event: BotEvent) {
  // 在这里写入业务自己的持久任务队列，勿执行长耗时网络调用。
  return { status: 'queued' };
}

router.bus.subscribe({
...
片段展示接线方式，通知队列需要业务实现；框架没有内置通知发送能力。回调接收 `BotEvent`，可读取 repo、number、delivery_id、kind、action、payload 等字段；支持同步或异步返回状态对象，也可不返回内容。结果对象可包含 hook / method，以保留归档 hooks 字段。

监听点 ID 必须唯一；空条件、无效回调或重复 ID 在注册时失败。启动后新增注册只影响下一次分发，当前投递使用已选定的监听快照；业务回调不阻塞新的监听注册。

| 字段 | 语义 |
| --- | --- |
| id / business / description | 稳定标识、所属业务、用途；同时用于监听点页面 |
| routes | 匹配`routeOf(event)`，多个规则取“或”；采用 shell 风格通配规则（`*`、`?`、字符集合），如 `issue.*` |
